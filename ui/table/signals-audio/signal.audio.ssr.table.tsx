...
type Props = { data: RawAudio[] };

export default function SignalementAudioSsrTableUI({ data }: Props) {
  ...
  const handleWarn = async (item: Signalement) => {
    try {
      const typeContent = "audios";
      const userID = item.audios.userId;
      const idContent = item.audios.id;
      ...
      toast.success(`Avertissement envoyé pour le contenu #${item.audios.id}`);
    } catch (err: any) {
      console.error(err);
      ...
      <div className="p-6">
        <Card className="border border-gray-200 shadow-sm">
          <CardBody className="text-center py-12">
            ...
            <p className="text-sm text-gray-500">Il n'y a actuellement aucun signalement audio à traiter</p>
          </CardBody>
        </Card>
      </div>
    );
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 mb-1">Signalements Audio</h1>
          <p className="text-sm text-gray-600">Gestion et suivi des signalements de contenu audio</p>
        </div>
      ...
      <Card className="border border-gray-200 shadow-sm">
        <CardBody className="p-0">
          <Table 
            aria-label="Liste des Signalements Audio" 
            isStriped
            removeWrapper
            ...
            >
              {signalements.map((item, index) => (
                ...
                >
                  ...
                  <TableCell className="py-4 px-6">
                    <span className="text-gray-800 font-medium text-sm">{item.audios?.titre ?? "-"}</span>
                  </TableCell>
                  <TableCell className="py-4 px-6">
                    <span className="text-gray-600 text-sm">{item.audios?.auteur ?? "-"}</span>
                  </TableCell>
                  <TableCell className="py-4 px-6">
                  ...
                  <TableCell className="py-4 px-6">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Button
                        ...
                        variant="solid"
                        as="a"
                        href={`https://ecclesiabook.org/audios/${item.audios.id}`}
                        target="_blank"
                        rel="noreferrer"
