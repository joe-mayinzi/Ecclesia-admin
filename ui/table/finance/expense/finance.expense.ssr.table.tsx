...
const INITIAL_VISIBLE_COLUMNS = ["date", "motif", "amount", "budget", "actions"];


export default function FinanceExpenseSsrTableUI({ initData, session }: { session: Session, initData: ManagementExpenses[] }) {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredEvents = filteredEvents.filter((event) =>
        ...
        event.budget?.budgetLine.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
  ...
  const renderCell = React.useCallback((expense: Expense, columnKey: React.Key) => {
    switch (columnKey) {
      ...
      case "actions":
        return (
          <ActionExpense session={session} handleFindExpense={handleFindExpense} expense={expense} />
        );
      default:
        return <></>;
    }
  ...
  const handleFindExpense = async () => {
    if (session) {
      const find = await findManagementExpensesByEgliseIdApi(session.user.eglise.id_eglise);
      if (find) {
        setEvents(find);
      }
    }
      ...
      <div className="flex flex-col gap-4">
        <div className="grid grid-cols-2 justify-between gap-3 mt-4">
          ...
          <div className="flex justify-end gap-3">
            ...
            <CreateExpenseFormModal session={session} handleFindExpense={handleFindExpense} />
          </div>
        </div>
